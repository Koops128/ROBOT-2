...
task detectLine() {
	bool online = false;
	while (1) {
		 if (SensorValue[colorLeft] < 10) { //black = 1
		   online = true;
		   setLEDColor(ledOrange);
		   stopTask(wander);
		   setMotorSpeed(motorLeft, 20);
		   setMotorSpeed(motorRight, 35);
		   while (SensorValue[colorRight] >= 10) {
		     wait(1);
		   }
		   setMotorSpeed(motorRight, 20);
	}
		   if (SensorValue[colorRight] < 10) { //black = 1
		   online = true;
		   setLEDColor(ledOrange);
		   stopTask(wander);
		   setMotorSpeed(motorLeft, 35);
		   setMotorSpeed(motorRight, 20);
		   while (SensorValue[colorLeft] >= 10) {
		     wait(1);
		   }
		   setMotorSpeed(motorLeft, 20);
		}
		   while (online) {
		     setLEDColor(ledGreen);
		   	while (SensorValue[colorLeft] < 10 && SensorValue[colorRight] < 10) {
					sleep(5);
				}
				if (SensorValue[colorLeft] >= 10) {
					setMotorSpeed(motorLeft, 35);
						while (SensorValue[colorLeft] >= 10) {
							sleep(3);
						}
...
task detectObject() {
	...
	while (1) {
		if (getUSDistance(S3) <= 3.0) {
				...
				running = false;
		} else if (getUSDistance(S3) <= 92.0) {
				stopTask(wander);
        running = false;
        // -(.1x - 10)^2 + 100
				int motorSpeed = -1 * ((.1 * getUSDistance(S3) - 10)*(.1 * getUSDistance(S3) - 10)) + 100;
